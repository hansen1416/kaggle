...
class KoreGymEnv(gym.Env):
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        # Ignore trainer reward, which is just delta kore
        self.raw_obs, _, done, info = self.trainer.step(kore_action)
    ...
    def gym_to_kore_action(self, gym_action: np.ndarray) -> Dict[str, str]:
        ...
        for shipyard in me.shipyards:
            ...
            if action_build:
                ...
                number_of_ships = min(
                    number_of_ships, max_spawn, max_purchasable)
                if number_of_ships:
                ...
                if number_of_ships:
                    # int between 0 (North) and 3 (West)
                    direction = round((gym_action[1] + 1) * 1.5)
    ...
    def obs_as_gym_state(self) -> np.ndarray:
        ...
        gym_state = np.ndarray(
            shape=(self.config.size, self.config.size, N_FEATURES))
        ...
        for point, cell in board.cells.items():
            ...
            if fleet:
                ...
                gym_state[point.y, point.x,
                          1] = gym_state[point.y, point.x, 2] = 0

            # Feature 3: Shipyard present (1: friendly, -1: enemy)
            shipyard = cell.shipyard
            if shipyard:
                gym_state[point.y, point.x,
                          3] = 1 if shipyard.player_id == our_id else -1
            else:
        ...
        progress = clip_normalize(
            board.step, low_in=0, high_in=GAME_CONFIG['episodeSteps'])
        my_kore = clip_normalize(
            np.log2(player.kore+1), low_in=0, high_in=np.log2(MAX_KORE_IN_RESERVE))
        opponent_kore = clip_normalize(
            np.log2(opponent.kore+1), low_in=0, high_in=np.log2(MAX_KORE_IN_RESERVE))
    ...
    def compute_reward(self, done: bool, strict=False) -> float:
        ...
        if strict:
            ...
            if done:
                ...
                win_reward = we_won * \
                    (WIN_REWARD + 5 *
                     (GAME_CONFIG['episodeSteps'] - board.step))
            else:
                win_reward = 0
