...
WEIGHTS_ASSETS = np.append(_weights_assets, np.zeros(
    _max_steps - _end_of_asset_value))
WEIGHTS_KORE = np.append(_weights_kore, np.ones(
    _max_steps - _end_of_asset_value))
# Value multiplier of a shipyard as a function of its max spawn
WEIGHTS_MAX_SPAWN = {x: (x+3)/4 for x in range(1, 11)}
# Always equal or smaller than either, almost always smaller
WEIGHTS_KORE_IN_FLEETS = WEIGHTS_KORE * WEIGHTS_ASSETS/2
...
def get_board_value(board: Board) -> float:
    ...
    step = board.step
    weight_kore, weight_assets, weight_cargo = WEIGHTS_KORE[
        step], WEIGHTS_ASSETS[step], WEIGHTS_KORE_IN_FLEETS[step]

    # Compute the partial board values
    for player in board.players.values():
        player_fleets, player_shipyards = list(
            player.fleets), list(player.shipyards)
        ...
        value_shipyards = weight_assets * SHIPYARD_COST * (
            sum(shipyard.max_spawn *
                WEIGHTS_MAX_SPAWN[shipyard.max_spawn] for shipyard in player_shipyards)
        )

        value_kore_in_cargo = weight_cargo * \
            sum(fleet.kore for fleet in player_fleets)
        ...
        board_value += modifier * \
            (value_kore + value_fleets + value_shipyards + value_kore_in_cargo)
